...
main()
{
    ...
    BigInteger ONE, ZERO;
    init_big_integer(&ONE, 1);
    init_big_integer(&ZERO, 0);
    ...
    while (1) {
        ...
        ttl = (BigInteger**) calloc((d + 1), sizeof(BigInteger*));
        for (int i = 0; i < d + 1; ++i) {
            ttl[i] = (BigInteger*) calloc((m + 1), sizeof(BigInteger));
        }

        copy_big_integer(&ONE, &ttl[1][m]);
        copy_big_integer(&ONE, &ttl[1][0]);

        for (int i = 2; i < d + 1; ++i) {
            for (int j = 1; j < m; ++j) {
                copy_big_integer(&ttl[i - 1][j - 1], &ttl[i][j]);
            }
            add_to(&ttl[i - 1][m],
                   &ttl[i - 1][m - 1],
                   &ttl[i][m]);
            copy_big_integer(&ttl[i][m], &ttl[i][0]);
        }

        BigInteger result;
        copy_big_integer(&ZERO, &result);

        for (int i = 0; i < m + 1; ++i) {
            BigInteger tmp;
            copy_big_integer(&result, &tmp);
            add_to(&tmp, &ttl[d][i], &result);
        }
